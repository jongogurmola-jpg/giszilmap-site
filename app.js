...
const BUILD = "1789458028";  // replaced with the publish timestamp by publish.sh
...
map.on("load", async () => {
  ...
  bgData = await (await fetch("tiles/blockgroups.geojson?v=1789458028" + DEVQ)).json();
  ...
  bgOrder = await fetch("tiles/bg_order.json?v=1789458028" + DEVQ)
    .then(r => r.ok ? r.json() : null).catch(() => null);
  taxProfiles = await fetch("tiles/tax_profiles.json?v=1789458028" + DEVQ)
    .then(r => r.ok ? r.json() : null).catch(() => null);
  ...
  map.addSource("counties", { type: "geojson", data: "tiles/counties.geojson?v=1789458028" + DEVQ });
  map.addLayer({
  ...
  map.addSource("crimetrend", { type: "geojson", data: "tiles/crime_trend.geojson?v=1789458028" + DEVQ });
  map.addLayer({
  ...
  map.addSource("parks", { type: "geojson", data: "tiles/parks.geojson?v=1789458028" + DEVQ });
  map.addLayer({
    id: "parks", type: "fill", source: "parks",
    paint: { "fill-color": "#008300", "fill-opacity": 0.35 },
  }, firstLabelLayer());

  map.addSource("amenities", { type: "geojson", data: "tiles/amenities.geojson?v=1789458028" + DEVQ });
  map.addLayer({
  ...
  map.addSource("grocery", { type: "geojson", data: "tiles/grocery.geojson?v=1789458028" + DEVQ });
  map.addLayer({
  ...
  map.addSource("worship", { type: "geojson", data: "tiles/worship.geojson?v=1789458028" + DEVQ });
  map.addLayer({
  ...
  map.addSource("stripclubs", { type: "geojson", data: "tiles/stripclubs.geojson?v=1789458028" + DEVQ });
  ...
  map.addSource("districts", { type: "geojson", data: "tiles/school_districts.geojson?v=1789458028" + DEVQ });
  map.addLayer({
  ...
  map.addSource("listings", { type: "geojson", data: "tiles/listings.geojson?v=1789458028" + DEVQ });
  map.addLayer({
  ...
  map.addSource("sold", { type: "geojson", data: "tiles/sold.geojson?v=1789458028" + DEVQ });
  map.addLayer({
  ...
  fetch("tiles/meta.json?v=1789458028" + DEVQ).then(r => r.ok ? r.json() : null).then(m => {
...
async function openDeepLink() {
  ...
  for (const file of ["tiles/listings.geojson?v=1789458028" + DEVQ, "tiles/sold.geojson?v=1789458028" + DEVQ]) {
